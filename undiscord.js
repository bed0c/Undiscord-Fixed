...
const maxEmptyPages = 3; // Maksimum atlanacak boş sayfa sayısı
...
async search() {
    ...
    let resp;
    let emptyPageCount = 0;  // Boş sayfa sayacı
    const maxEmptyPages = 3; // Maksimum atlanacak boş sayfa sayısı
    ...
    // API yanıtı 202 (henüz indekslenmemiş kanal)
    if (resp.status === 202) {
        let waitTime = (await resp.json()).retry_after * 1000 || this.stats.searchDelay;
        this.stats.throttledCount++;
        this.stats.throttledTotalTime += waitTime;
        log.warn(`Bu kanal henüz indekslenmedi. ${waitTime}ms bekleniyor...`);
        await wait(waitTime);
    ...
    if (!resp.ok) {
        if (resp.status === 429) {
            ...
            this.stats.throttledCount++;
            this.stats.throttledTotalTime += waitTime;
            this.stats.searchDelay += waitTime; // Gecikmeyi artır
            log.warn(`API tarafından ${waitTime}ms süreyle kısıtlandınız! Arama gecikmesi artırılıyor...`);
            this.printStats();
            await wait(waitTime * 2);
            return await this.search();
        } else {
            this.state.running = false;
            log.error(`Hata! API yanıtı: ${resp.status}`, await resp.json());
            throw resp;
        }
    }
    ...
    if (!data.messages || data.messages.length === 0) {
        emptyPageCount++;
        log.warn(`Boş sayfa alındı! (${emptyPageCount}/${maxEmptyPages})`);
        
        if (emptyPageCount >= maxEmptyPages) {
            log.error('Çok fazla boş sayfa alındı, işlem durduruluyor.');
            this.state.running = false;
            return;
        } else {
            await wait(2000); // 2 saniye bekle ve tekrar dene
            return await this.search();
        }
    } else {
        emptyPageCount = 0; // Eğer boş değilse sıfırla
    }
	  ...
	  async deleteMessage(message) {
	    ...
	    if (!resp.ok) {
	      if (resp.status === 429) {
	        ...
	        this.options.deleteDelay = 1000; // increase delay
